...
|---|---|---|
| `unicode1` (a)(b)(c) 三个一句话回答 | 已完成 | `writeup.md` 1.1 |
| `unicode2` (a)(b)(c) 三个回答 | 已完成 | `writeup.md` 1.2 |
...
| `transformer_accounting` (a)(b)(c)(d)(e) | 已完成 | `writeup.md` 2.1 |
| `cross_entropy` | 已完成 | `cs336_basics/nn_utils.py`，`tests/test_nn_utils.py` |
| `learning_rate_tuning` (toy SGD) | 已完成 | `writeup.md` 2.2 |
| `adamw`：实现优化器 | 已完成 | `cs336_basics/optim.py`，`tests/test_optimizer.py` |
| `adamwAccounting` (a)(b)(c)(d) | 已完成 | `writeup.md` 2.3 |
...
| `learning_rate`：多学习率曲线+策略 | 已完成 | `artifacts/experiments/lm/lr_sweep_summary.*`，`artifacts/figures/tinystories_lr_sweep_*.png`，`writeup.md` 3.2 |
| `learning_rate`：TinyStories val loss ≤ 1.45 | 已完成 | `artifacts/experiments/lm/ts_main_lr1e3_20k/best_metrics.json`（1.373） |
| `learning_rate`：稳定边界/发散分析（含 divergent run） | 已完成 | `artifacts/experiments/lm/lr_divergence_summary.*`，`artifacts/figures/tinystories_lr_divergence_val.png`，`slurm_logs/train_lm_1478.out`，`slurm_logs/train_lm_1479.out` |
| `batch_size`：不同 batch 曲线 | 已完成 | `artifacts/experiments/lm/batch_size_summary.*`，`artifacts/figures/tinystories_batch_size_val.png` |
| `batch_size`：结论分析 | 已完成 | `writeup.md` 3.3 |
| `batch_size`：从 1 到显存上限 | 已完成 | `artifacts/experiments/lm/batch_size_summary.md`（含 1/32/64/128/256）+ `slurm_logs/train_lm_1475.err`（batch=512 OOM） |
| `generate`：TinyStories 文本生成与评述 | 已完成 | `artifacts/experiments/lm/ts_main_lr1e3_20k/generated.txt`，`writeup.md` 3.4 |
...
| `layer_norm_ablation`：影响分析 | 已完成 | `writeup.md` 3.5 |
...
| `swiglu_ablation`：SwiGLU vs SiLU 曲线（参数量匹配） | 已完成 | `artifacts/experiments/lm/ts_ablate_silu_dff2048_lr1e3_5k/config.json`（`d_ff=2048`）+ `artifacts/figures/tinystories_ablations_val.png`，`ablation_summary.*` |
| `mainExperiment`：OWT 学习曲线与损失解释 | 已完成 | `artifacts/experiments/lm/owt_main_lr1e3_20k/*`，`artifacts/figures/ts_vs_owt_main_val.png`，`writeup.md` 3.6 |
| `mainExperiment`：OWT 生成文本与分析 | 已完成 | `artifacts/experiments/lm/owt_main_lr1e3_20k/generated.txt`，`writeup.md` 3.4/3.6 |
